...
This is the original template README preserved for reference.
The current project README lives at the repo root.
...
The competition endpoint and local smoke-test instructions live in [docs/test_API.md](../test_API.md).
That file explains how to run the FastAPI server locally and how the request flow connects into the existing `decision_making` workflow.
The shortest path is usually `make api-test`, `make api-integration-test`, or `make api-server` if `make` is installed.
