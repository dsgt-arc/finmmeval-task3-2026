...
class PlannerOutput(BaseModel):
    ...
    justification: str = Field(
        description="Explanation for the analyst selection", default="No justification provided due to error"
    )
...
def planner_agent(ticker: str, llm_config: Dict[str, Any], workflow_analysts: List[str]) -> List[str]:
    ...
    analyst_info = [{"analyst_name": key, "analyst_info": AgentRegistry.get_analyst_info(key)} for key in workflow_analysts]

    prompt = PLANNER_PROMPT.format(ticker=ticker, analysts=analyst_info)

    result = agent_call(prompt=prompt, llm_config=llm_config, pydantic_model=PlannerOutput)
